...
class BaseTests(DjangoTestCase):
    ...
    def setUp(self):
        ...
        for index, row in reviews[:50].iterrows():
            if is_not_null(row['text']):
                obj2 = SecondTestModel.objects.create(text_field=row["text"][:200], id=index)
                obj = TestModel.objects.create(text_field=row["text"][:200], second_related_object=obj2, id=index)
    ...
    def test_filter_field_dict(self):
        ...
        self.assertEqual(result, {'bioguide_id': None, 'icpsr_id': '12345', 'fec_ids': []})
        ...
        self.assertEqual(result, {'party__name': 'Democratic Party', 'icpsr_id': '12345', 'fec_ids': []})
    ...
    def test_tfidf_similarity(self):
        ...
        self.assertAlmostEqual(result, .74, 2)
        ...
        self.assertAlmostEqual(round(result[0]['similarity'], 2), .21, 2)

        result = TestModel.objects.all().tfidf_similarities(["text_field"], "quick movie review")
        self.assertEqual(result[0]['pk'], 1)
        self.assertAlmostEqual(round(result[0]['similarity'], 2), .35, 2)

        result = TestModel.objects.all().tfidf_similarity_best_match(["text_field"], "quick movie review")
        self.assertEqual(result['pk'], 1)
        self.assertAlmostEqual(round(result['similarity'], 2), .35, 2)
    ...
    def test_levenshtein_difference(self):
        ...
        self.assertEqual(result['difference'], .74)
